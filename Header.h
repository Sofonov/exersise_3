...
template <typename T>
void foo(T & aModel, std::list<ModelData_SceneGraphElement*> & plist, int deep=1)
{
...
template <typename T>
void foo(T & aModel, std::set<ModelData_Part*> & pset)
{
	ModelData_Model::ElementIterator anIterator(aModel);
	while (anIterator.HasNext())
	{
		...
		{	pset.insert(static_cast<ModelData_Part*> (&aSGE));
			return;
		}
